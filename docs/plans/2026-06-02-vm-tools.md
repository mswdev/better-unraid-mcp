...
Use the **MCP SDK client** (a dependency already) so the `initialize` →
`notifications/initialized` → `tools/list` handshake is done correctly — a
hand-rolled JSON-RPC snippet that sends `tools/list` before `notifications/initialized`
can be rejected with "Server not initialized" and falsely report FAIL. The server
requires `UNRAID_API_URL` + `UNRAID_API_KEY` at startup (no network call for
`tools/list`), so pass dummy values. Build output is `dist/index.js` (confirmed
from `tsup.config.ts` + `package.json` `bin`).
...
const transport = new StdioClientTransport({
  command: "node",
  args: ["dist/index.js"],
  env: { ...process.env, MCP_TRANSPORT: "stdio", UNRAID_API_URL: "http://localhost/graphql", UNRAID_API_KEY: "dummy" },
});
const client = new Client({ name: "smoke", version: "0" });
await client.connect(transport);
const { tools } = await client.listTools();
const names = tools.map((t) => t.name);
const ok = names.includes("vm_list") && names.includes("vm_action");
await client.close();
console.log(ok ? "SMOKE PASS: vm_list + vm_action listed" : "SMOKE FAIL: " + names.join(", "));
process.exit(ok ? 0 : 1);
'
```
Expected: `SMOKE PASS: vm_list + vm_action listed`.
