...
Expected: `src/types/unraid/graphql.ts` created, exporting **exactly** `GetSystemInfoQuery`, `GetSystemInfoQueryVariables`, and `GetSystemInfoDocument: TypedDocumentNode<GetSystemInfoQuery, GetSystemInfoQueryVariables>`. **Verify these exact export names** before anything imports them (later tasks depend on them). Do **not** hand-edit this file or add a banner — graphql-codegen emits its own header, and Task 10's CI re-runs `npm run generate` and fails on any diff. If you want a custom header, add it via codegen's `add` plugin so regeneration reproduces it byte-for-byte.
...
**Step 5: Report** the PR URL and the green quality-gate output to the user. **Be honest about coverage:** the stdio smoke test only exercises `tools/list`, which never calls the Unraid API. `get_system_info` is therefore unproven against a live server — the query matches the SDL, but permissions or field nullability could still surprise on first real contact. List **"run `get_system_info` against a real Unraid box"** as the first post-merge check; do not imply the tool is verified end-to-end.
